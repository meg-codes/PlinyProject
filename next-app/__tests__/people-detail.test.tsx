...
describe('Person Detail', () => {
  ...
  it('should render a message if an empty object is passed as a prop', () => {
    const persondetail = shallow(<PersonDetail id={1} person={undefined}></PersonDetail>)
    expect(persondetail.find('h1').text()).toEqual('No person data loaded.');
  });

  it('should render api data passed as a prop', () => {
    const persondetail = shallow(<PersonDetail id={1} person={tacitus}></PersonDetail>);
  ...
  it('should handle all dates that may be passed to it', () => {
    const persondetail = shallow(<PersonDetail id={1} person={fakeDatesTacitus}></PersonDetail>);
    expect(persondetail.find('.person-details').html()).toMatchSnapshot();
  });
  ...
  it('should handle a call on the server side', () => {
    ...
    const context = {
      ...
      req: {
        protocol: 'http',
        get: (key: string) => { if (key === 'Host') return 'localhost:3000' },
    ...
    mock
      .onGet('http://localhost:3000/api/people/1')
      .reply(200, tacitus)
    if (context.res) {
      expect.assertions(4);
    } else {
  ...
  it('should rewrite the url if a request for wrong full url comes in', () => {
    ...
    return PersonDetail.getInitialProps(context).then(() => {
      if (context.res) {
        expect(context.res.writeHead).toBeCalledWith(302, {Location: '/people/cornelius-tacitus-1'});
